  ...
  const executeEnrollment = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 402) {
          // Step 2: Payment is strictly required. 
          if (selectedCardId && selectedCardId !== "NEW") {
            ...
            // Instantly route to learning portal
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-900 rounded-3xl p-8 max-w-sm w-full shadow-2xl border border-gray-100 dark:border-gray-800 transform transition-all">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6 leading-relaxed">
              Are you sure you want to proceed? You will be charged <span className="font-bold text-[#2D6A4F] text-lg">₦{price?.toLocaleString() || "..."}</span>
            </p>
              ...
              <div className="mb-8 flex flex-col gap-3">
                {savedCards.length > 0 && (
                  <div className="flex flex-col gap-2">
                    ...
                    <label className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${selectedCardId === "NEW" ? "border-[#2D6A4F] bg-[#2D6A4F]/5 dark:bg-[#52b788]/10" : "border-gray-200 dark:border-gray-800 hover:border-gray-300"}`}>
                      <input type="radio" name="paymentMethod" checked={selectedCardId === "NEW"} onChange={() => setSelectedCardId("NEW")} className="w-4 h-4 text-[#2D6A4F]" />
                ...
                {selectedCardId === "NEW" && (
                  <label className="flex items-start gap-2 cursor-pointer mt-2 bg-gray-50 dark:bg-gray-800/50 p-3 rounded-xl border border-gray-100 dark:border-gray-800">
                    <input type="checkbox" checked={saveNewCard} onChange={(e) => setSaveNewCard(e.target.checked)} className="w-4 h-4 mt-0.5 rounded border-gray-300 text-[#2D6A4F] focus:ring-[#2D6A4F]" />
