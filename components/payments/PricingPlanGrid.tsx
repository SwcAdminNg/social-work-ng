...
export function PricingPlanGrid({ plans, isLoggedIn, currentSubscription }: PricingPlanGridProps) {
  ...
  const handleSubscribeClick = async (planId: string, planName: string, price: number) => {
    ...
    try {
      if (hasActiveSubscription) {
        ...
        if (selectedCardId && selectedCardId !== "NEW") {
          ...
          if (!chargeRes.ok) throw new Error(chargeData.message || "Failed to charge saved card.");
          
          window.location.href = "/payment-confirmation?reference=" + (chargeData.data?.reference || "success");
          return;
        } else {
          ...
          >
            ...
            <div className="mt-auto">
              <SubscriptionButton 
                ...
                onSubscribeClick={() => handleSubscribeClick(plan.id, plan.name, plan.price)}
              />
            </div>
          </div>
        );
          ...
          <div className="bg-white dark:bg-gray-900 rounded-3xl p-8 max-w-sm w-full shadow-2xl border border-gray-100 dark:border-gray-800 transform transition-all">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6 leading-relaxed">
              {hasActiveSubscription ? (
                <>
                  Your plan will securely change to <span className="font-bold text-gray-900 dark:text-white">{selectedPlan.name}</span> at the end of your current billing cycle. You will be charged <span className="font-bold text-[#52b788]">₦{selectedPlan.price.toLocaleString()}</span> upon renewal.
                </>
              ) : (
                <>
                  You are subscribing to the <span className="font-bold text-gray-900 dark:text-white">{selectedPlan.name}</span> plan. You will be charged <span className="font-bold text-[#52b788] text-xl block mt-2">₦{selectedPlan.price.toLocaleString()}</span>
                </>
              )}
            </p>
              ...
              <div className="mb-8 text-left flex flex-col gap-3">
                {savedCards.length > 0 && (
                  <div className="flex flex-col gap-2">
                    <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Select Payment Method</p>
                    {savedCards.map(card => (
                      <label key={card.id} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${selectedCardId === card.id ? "border-[#52b788] bg-[#52b788]/10" : "border-gray-200 dark:border-gray-800 hover:border-gray-300"}`}>
                        <input type="radio" name="paymentMethod" checked={selectedCardId === card.id} onChange={() => setSelectedCardId(card.id)} className="w-4 h-4 text-[#52b788]" />
                        <span className="font-medium text-gray-900 dark:text-white capitalize">{card.brand} ending in •••• {card.last4}</span>
                      </label>
                    ))}
                    <label className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${selectedCardId === "NEW" ? "border-[#52b788] bg-[#52b788]/10" : "border-gray-200 dark:border-gray-800 hover:border-gray-300"}`}>
                      <input type="radio" name="paymentMethod" checked={selectedCardId === "NEW"} onChange={() => setSelectedCardId("NEW")} className="w-4 h-4 text-[#52b788]" />
                      <span className="font-medium text-gray-900 dark:text-white">Pay with a new card</span>
                    </label>
                  </div>
                )}
                ...
                {selectedCardId === "NEW" && (
                  <label className="flex items-start gap-2 cursor-pointer mt-2 bg-gray-50 dark:bg-gray-800/50 p-3 rounded-xl border border-gray-100 dark:border-gray-800">
                    <input type="checkbox" checked={saveNewCard} onChange={(e) => setSaveNewCard(e.target.checked)} className="w-4 h-4 mt-0.5 rounded border-gray-300 text-[#52b788] focus:ring-[#52b788]" />
                    <span className="text-sm text-gray-600 dark:text-gray-400">Save this card for future 1-click purchases</span>
                  </label>
                )}
              </div>
            ) : null}
